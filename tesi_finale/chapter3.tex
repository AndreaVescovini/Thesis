...
This approach is known also as Marker and Cell (MAC), as it was names in its 
...
%a bias of the value on the face towards the direction where the flow comes from must occur.
From a mathematical 
...
In the following we will list the most straight-forward options for linear schemes, 
referring to the 
...
%kinetic energy $k$ or its specific dissipation rate $\omega$.
%After having 
...
%\begin{multline}
%	\frac{\partial \phi}{\partial t} \approx \frac{\phi^n\Delta 
%	t^{n-1}(2\Delta t^n+\Delta t^{n-1})-\phi^{n-1}(\Delta t^n+\Delta 
%	t^{n-1})^2+\phi^{n-2}(\Delta t^n)^2}{\Delta 
%	t^n\Delta t^{n-1}(\Delta t^n+\Delta t^{n-1})} =\\
%= f(\phi^n, t^n)
%\end{multline}
...
%The degrees of freedom $u_\text{ff}^\text{if}$ and $p_\text{pm}^\text{if}$ will be 
%useful in the following computations, but are fictitious,
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{coupled_cells_axis.pdf}
...
\begin{equation}
[\mathbf{J}_F]_{ij} = \frac{F_i([\mathbf{x}_k^n]_j + \varepsilon) - F_i([\mathbf{x}_k^n]_j - \varepsilon)}{2\varepsilon},
\end{equation}
where $\varepsilon=\num{e-8}|[\mathbf{x}_k^n]_j|$.
