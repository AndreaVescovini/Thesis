...
\label{eq:nsmom} \frac{\partial{(\varrho \mathbf{v}})}{\partial t} + \nabla 
...
many three-dimensional eddies, that enhance the diffusive and mixing properties 
of the fluid. 
...
\label{eq:ransmom} \frac{\partial{(\varrho \bar{\mathbf{v}})}}{\partial t} 
+ 
\nabla 
\cdot (\varrho \bar{\mathbf{v}} \bar{\mathbf{v}}^\mathrm{T}) + \nabla 
\cdot 
(\varrho \overline{\mathbf{v}' {\mathbf{v}'}^\mathrm{T}})- \nabla \cdot (\mu 
\nabla 
\bar{\mathbf{v}}) + \nabla \bar{p} - \varrho\mathbf{g} = \mathbf{0}&
\end{align}
...
turbulent viscosity $\mu_t$ [$\si{\pascal \second}$]. The second term involves 
the is needed to model correctly also the isotropic part of $\tau_R$, indeed:
...
\nabla 
\cdot (\varrho \bar{\mathbf{v}} \bar{\mathbf{v}}^\mathrm{T}) - \nabla \cdot 
(\mu_\text{eff} \nabla \bar{\mathbf{v}}) + \nabla (\bar{p} + \frac{2}{3}\varrho 
k) 
-\varrho\mathbf{g} = \mathbf{0},
\end{equation}
where we have introduced an effective viscosity
\begin{equation}
	\mu_\text{eff} = \mu + \mu_t.
\end{equation}
...
\end{equation}
reducing the unknowns to $\mu_t$ alone, for which we can employ different 
...
\begin{equation}
\nu_t = \frac{\mu_t}{\varrho} = l_\text{mix}v_\text{mix},
\end{equation}
...
theory \eqref{eq:loglaw} in order to compute the velocities at the cell near 
the wall. This can be done easily for flat walls and it brings also 
computational advantages because we can avoid to have very small cells near the 
wall that solve until the viscous sublayer.

Many variants of this model have been developed, for example the 
\emph{low-$Re$} $k\text{-}\varepsilon$ model that adds extra terms in order to 
model correctly the behaviour near the wall. See \cite{main:vermal} for more 
information.
%
...
%This model works also near the boundary... we can not fix $\omega$ at the 
%boundary but we fix it a bit inside.
...
\begin{itemize}
	\item the continuity of normal mass fluxes:
	\begin{equation}
