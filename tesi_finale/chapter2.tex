...
example the gravity $\mathbf{g}$, $\boldsymbol{\sigma}$ is the Cauchy stress tensor 
and 
...
tensor:
\begin{equation}
\boldsymbol{\sigma} = -p\mathbb{1} + \boldsymbol{\tau},
\end{equation}
$-p\mathbb{1}$ is a contribution due to pressure, while $\boldsymbol{\tau}$ is 
...
\begin{itemize}
	...
	\begin{equation}
		\boldsymbol{\sigma} \mathbf{n} = -p\mathbf{n}+2\mu \mathbf{Sn} = \varrho\mathbf{d},
	\end{equation}
...
The new term
\begin{equation}
\boldsymbol{\tau}_R = -\varrho \overline{\mathbf{v}' {\mathbf{v}'}^\mathrm{T}}
\end{equation}
...
The quantity $\boldsymbol{\tau}_R$ is a symmetric tensor with $dim \times dim$ entries, so 
in three 
...
In 1877 \textcite{ipo:boussinesq} suggested that $\boldsymbol{\tau}_R$ could be similar to the viscous 
stress tensor, thus expressed as
\begin{equation} \label{eq:bouss}
\boldsymbol{\tau}_R = 2\mu_t \bar{\mathbf{S}} - 
\frac{2}{3}\varrho k \mathbb{1}.
\end{equation}
The first term has the same form of $\boldsymbol{\tau}$ if we consider the mean velocity 
field, 
...
to model the isotropic part of $\boldsymbol{\tau}_R$ correctly:
\begin{equation} \label{eq:tracciatau}
tr(\boldsymbol{\tau}_R) = -tr(\varrho \overline{\mathbf{v}' {\mathbf{v}'}^\mathrm{T}}) = 
...
\begin{equation}
	tr(2\mu_t \bar{\mathbf{S}}) = 2 \mu_t \sum_{i=1}^{dim} \frac{\partial 
