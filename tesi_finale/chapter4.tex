...
\begin{figure}
	...
	The grey lines are the reference lines for the first-order (lower) and 
	second-order (upper) convergence.}
	\label{fig:sin_err}
\end{figure}
...
Between the Min-Mod and Van Leer flux limiter it is hard to say which one 
...
spatial error is dominant over the temporal one, so that, even with small 
...
\begin{figure}
	...
	grey lines are the reference lines for the first-order (lower) and 
	second-order (upper) convergence.}
	\label{fig:timeerr}
\end{figure}
%
...
reference solution. Indeed, looking at the $L^2(\Omega)$ norm of the errors 
reported in 
...
exception of the channel length before the step. Indeed, in \cite{web:nasa}, a 
...
channel length in order to match as good as possible the profile of the $u$ 
component of the velocity along the cross-section at the position $x/H=-4$, 
...
For what concerns the reattachment length, we obtain a good agreement with the 
CFL3D code, indeed, looking at Figure~\ref{fig:bfscf}, with the Van Leer flux 
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{coupled_first_cavity_report.png}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{coupled_second_cavity_report.png}
